...
export function openStore({ databasePath, logger = console }) {
  ...
  function migrate() {
    db.exec(`
      ...
      CREATE TABLE IF NOT EXISTS docs (
        ...
        -- 内嵌图片是否已处理过：新开开关时用它决定"哪些老文件需要补做"，不必整库重建
        media_ocr INTEGER NOT NULL DEFAULT 0
      );
      ...
      -- 内嵌图片的 OCR 结果缓存（键是图片内容 sha1）：重建索引时不必把同一张图再 OCR 一遍
    ...
    `);
    // 老库补列：同步记录要能存"这次同步了哪些文件"的逐文件明细
    const logColumns = db.prepare('PRAGMA table_info(sync_log)').all().map(row => String(row.name));
    if (!logColumns.includes('details')) db.exec('ALTER TABLE sync_log ADD COLUMN details TEXT');
    // 老库补列：内嵌图片是否处理过（用于"开了开关就自动补做"）
    const docColumns = db.prepare('PRAGMA table_info(docs)').all().map(row => String(row.name));
    if (!docColumns.includes('media_ocr')) db.exec('ALTER TABLE docs ADD COLUMN media_ocr INTEGER NOT NULL DEFAULT 0');
  ...
  const listDocuments = () => db.prepare(
    'SELECT id, path, rel, dir, name, ext, size, mtime, sha1, kind, text_source, chars, pages, status, error, indexed_at, tries, media_ocr FROM docs',
  ).all();
    ...
    try {
      ...
      if (existing === undefined) {
        const result = db.prepare(`
          INSERT INTO docs (path, rel, dir, name, ext, size, mtime, sha1, kind, text_source, chars, pages, status, error, indexed_at, tries, media_ocr)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(
        ...
        db.prepare(`
          UPDATE docs SET rel = ?, dir = ?, name = ?, ext = ?, size = ?, mtime = ?, sha1 = ?,
            kind = ?, text_source = ?, chars = ?, pages = ?, status = ?, error = ?, indexed_at = ?, tries = ?, media_ocr = ?
          WHERE id = ?
        `).run(
          doc.rel, doc.dir, doc.name, doc.ext, doc.size, doc.mtime, doc.sha1 ?? null,
          doc.kind, doc.textSource ?? 'native', doc.chars ?? 0, doc.pages ?? null,
          doc.status ?? 'indexed', doc.error ?? null, Date.now(), doc.tries ?? 0,
          doc.mediaOcr === true ? 1 : 0, docId,
        );
  ...
  function buildFilters({ dir, ext, days }) {
    const where = [];
    const args = [];
    ...
    if (Number.isFinite(wanted) && wanted > 0) {
      where.push('d.mtime >= ?');
      args.push(Date.now() - wanted * 86400000);
    }
  ...
  function search(plan, { limit = 8, maxPerDoc = 3, dir, ext, days, candidateLimit = 80 } = {}) {
  ...
  // ---- 内嵌图片 OCR 缓存：按图片内容 sha1 命中，重建索引时不重复 OCR ----
  const getImageOcr = (sha1) => {
    const row = db.prepare('SELECT text FROM image_ocr WHERE sha1 = ?').get(String(sha1));
    return row === undefined ? undefined : String(row.text);
  };
  const setImageOcr = (sha1, text, bytes = 0) => {
    db.prepare(`
      INSERT INTO image_ocr(sha1, text, bytes, at) VALUES(?, ?, ?, ?)
      ON CONFLICT(sha1) DO UPDATE SET text = excluded.text, at = excluded.at
    `).run(String(sha1), String(text).slice(0, 200000), Number(bytes) || 0, Date.now());
    // 只保留最近 5000 张，避免缓存无限膨胀
    db.prepare('DELETE FROM image_ocr WHERE sha1 NOT IN (SELECT sha1 FROM image_ocr ORDER BY at DESC LIMIT 5000)').run();
  };
  const imageOcrStats = () => {
    const row = db.prepare('SELECT COUNT(*) AS n, COALESCE(SUM(LENGTH(text)), 0) AS chars FROM image_ocr').get();
    return { images: Number(row.n), chars: Number(row.chars) };
  };

  const sessionGet = (sessionId) => {    const row = db.prepare('SELECT enabled FROM session_state WHERE session_id = ?').get(String(sessionId));
  ...
  const logSync = (level, message, details = null) => {
    const payload = details === null || details === undefined
      ? null
      : JSON.stringify(details).slice(0, 20000);
    db.prepare('INSERT INTO sync_log(ts, level, message, details) VALUES(?, ?, ?, ?)')
      .run(Date.now(), level, String(message).slice(0, 4000), payload);
  ...
  const recentLogs = (limit = 30) => db.prepare('SELECT ts, level, message, details FROM sync_log ORDER BY id DESC LIMIT ?')
    .all(limit)
    .map(row => ({
      ts: Number(row.ts),
