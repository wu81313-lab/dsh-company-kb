...
/** 扫描原语：渲染器（lib/render/*）也复用这两个，避免两份实现漂移。 */
export function indexOfElementStart(xml, name, from) {
  const token = `<${name}`;
...
export function findElementEnd(xml, name, from) {
  const endToken = `</${name}>`;
  let depth = 0;
  let cursor = from;
  while (cursor < xml.length) {
    ...
    if (open !== -1 && open < close) {
      // 自闭合的 <w:p/> 没有结束标签，算进深度会让配对永远回不到 0
...
/**
 * 取一个元素（含自身）的 [start, end)。自闭合元素（`<w:p/>`）直接返回标签本身。
 * 抽取器与渲染器都走这里，避免两处配对逻辑各修一遍。
 */
export function elementRange(xml, name, at) {
  const gt = xml.indexOf('>', at);
  if (gt !== -1 && xml[gt - 1] === '/') return { start: at, end: gt + 1 };
  const end = findElementEnd(xml, name, at);
  return { start: at, end: end === -1 ? xml.length : end + name.length + 3 };
}
...
/** 渲染器复用：段落级别 + 纯文本标题模式。 */
export function headingLevelOf(paragraphXml, text) {
  const level = paragraphLevel(paragraphXml);
  if (level > 0) return level;
  return HEADING_TEXT.test(text) && text.length <= 80 ? 2 : 0;
}
...
function parseBody(xml, blocks) {
  ...
  while (index < xml.length) {
    ...
    if (at === nextTable && (nextPara === -1 || nextTable < nextPara)) {
      const range = elementRange(xml, 'w:tbl', at);
      const slice = xml.slice(range.start, range.end);
    ...
    const range = elementRange(xml, 'w:p', at);
    const slice = xml.slice(range.start, range.end);
    const text = paragraphText(slice).trim();
    if (text.length > 0) {
      const finalLevel = headingLevelOf(slice, text);
