...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // ------------------------------------------------ 预览布局（可拖 / 可记忆）
    // 只存"用户的选择"，不存派生值；读的时候一律夹紧，脏数据不会把界面搞坏。
    const LAYOUT_KEY = 'dslkb.layout.v1';
    const SPLIT_RANGE = [26, 80];      // 结果列表占的百分比
    const HEIGHT_RANGE = [320, 1200];  // 预览高度（px）
    const SPLIT_DEFAULT = 36;
    const HEIGHT_PRESETS = [
      { key: 'compact', label: '紧凑', value: 420 },
      { key: 'normal', label: '标准', value: 620 },
      { key: 'tall', label: '加高', value: 900 },
    ];

    /** 空串/null 当"没给值"处理，其余有限数字一律夹到区间内（负数夹到下限）。 */
    ...
    function clampSplit(value) {
      ...
      if (!Number.isFinite(number)) return SPLIT_DEFAULT;
      return Math.min(SPLIT_RANGE[1], Math.max(SPLIT_RANGE[0], Math.round(number)));
    }

    function clampHeight(value) {
      const number = toFinite(value);
      if (!Number.isFinite(number)) return 620;
      return Math.min(HEIGHT_RANGE[1], Math.max(HEIGHT_RANGE[0], Math.round(number)));
    }

    /** 默认高度跟窗口走：小窗口也要比"视口减 290"那套高一点。 */
    function defaultHeight(viewportHeight) {
      const height = Number(viewportHeight);
      if (!Number.isFinite(height) || height <= 0) return 620;
      return clampHeight(Math.min(720, Math.max(360, height - 260)));
    }

    /** 从 storage 读布局：非法 JSON / 缺字段 / 越界值都安全回落。 */
    function readLayout(storage) {
      const fallback = { ratio: SPLIT_DEFAULT, height: null };
      ...
      return {
        ratio: parsed.ratio === undefined ? SPLIT_DEFAULT : clampSplit(parsed.ratio),
        height: Number.isFinite(height) && height > 0 ? clampHeight(height) : null,
      };
    }
    ...
    function writeLayout(storage, layout) {
      ...
      try {
        storage.setItem(LAYOUT_KEY, JSON.stringify({
          ratio: clampSplit(layout.ratio),
          height: layout.height === null || layout.height === undefined ? null : clampHeight(layout.height),
        }));
      } catch { /* 隐私模式或配额满：记不住就算了，不影响使用 */ }
    }

    /** 能拿到就用 localStorage；隐私模式下访问本身就可能抛，所以裹一层。 */
    const layoutStorage = (() => {
      try {
...
.dslkb-filters{display:flex;gap:6px;align-items:center;margin-top:8px;flex-wrap:wrap}
.dslkb-filters .dslkb-input{flex:0 1 auto;width:auto;min-width:96px;height:32px;font-size:12.5px;padding:0 8px}
.dslkb-filters .dslkb-input[type="text"],.dslkb-filters input.dslkb-input{min-width:150px;flex:1 1 150px}

/* ------------------------------------------------------ 结果 + 详情分栏 */
/* 详情不再排在结果列表下面（那样每次点开都要滚到底）。宽屏时左右分栏：
   左边结果、右边详情；中间是拖拽手柄，宽度比例由 --kb-split 控制（用户可拖、可记忆）。
   窄屏自动退回单列（媒体查询在样式表后面，优先级高于基础规则），并由 JS 把详情滚进视野。 */
.dslkb-split{display:grid;grid-template-columns:minmax(0,1fr);gap:16px;align-items:start;margin-top:12px}
.dslkb-split[data-split="true"]{grid-template-columns:minmax(200px,var(--kb-split,36%)) 8px minmax(0,1fr);gap:0}
.dslkb-split[data-dragging="true"]{user-select:none;cursor:col-resize}
/* 满屏（沉浸）：藏掉结果列表与手柄，预览独占 */
.dslkb-split[data-immersive="true"]{grid-template-columns:minmax(0,1fr)}
.dslkb-split[data-immersive="true"] .dslkb-pane-list,
.dslkb-split[data-immersive="true"] .dslkb-split-handle{display:none}
.dslkb-pane-list{min-width:0;display:flex;flex-direction:column;gap:8px;padding-right:14px}
.dslkb-pane-detail{min-width:0;max-height:none;overscroll-behavior:contain}
.dslkb-split-handle{
  align-self:stretch;justify-self:center;width:8px;border-radius:5px;cursor:col-resize;
  background:linear-gradient(to right,transparent 3px,var(--kb-border) 3px,var(--kb-border) 5px,transparent 5px);
  transition:background-color .14s var(--kb-ease)
}
.dslkb-split-handle:hover,.dslkb-split-handle:focus-visible{
  outline:none;background:linear-gradient(to right,transparent 3px,var(--kb-border-strong) 3px,var(--kb-border-strong) 5px,transparent 5px)
}
@media (max-width:1100px){
  .dslkb-split[data-split="true"]{grid-template-columns:minmax(0,1fr);gap:16px}
  .dslkb-pane-detail{max-height:none}
  .dslkb-split-handle{display:none}
  .dslkb-pane-list{padding-right:0}
}
...
.dslkb-hitnav{display:flex;align-items:center;gap:8px;margin:6px 0 4px;flex-wrap:wrap}
.dslkb-media{width:100%;border:1px solid var(--kb-border);border-radius:10px;background:#fff;display:block}
/* 预览高度由用户控制：--kb-preview-h 是拖出来的像素值，iframe 撑满这个高度 */
.dslkb-preview-body{position:relative;height:var(--kb-preview-h,620px);min-height:200px}
.dslkb-preview-body .dslkb-media{height:100%}
.dslkb-preview-body.dslkb-preview-fill{height:calc(100vh - 200px)}
.dslkb-resize-y{
  position:absolute;left:0;right:0;bottom:-5px;height:11px;cursor:ns-resize;border-radius:6px;
  background:linear-gradient(to bottom,transparent 4px,var(--kb-border) 4px,var(--kb-border) 6px,transparent 6px)
}
.dslkb-resize-y:hover,.dslkb-resize-y:focus-visible{
  outline:none;background:linear-gradient(to bottom,transparent 4px,var(--kb-border-strong) 4px,var(--kb-border-strong) 6px,transparent 6px)
}
.dslkb-preview-head .dslkb-seg{margin-left:auto}
...
`;
    ...
    function Panel() {
      ...
      // 预览布局：比例来自 localStorage，高度为 null 时用"跟窗口走"的默认值
      const [layout, setLayout] = React.useState(() => readLayout(layoutStorage));
      const [drag, setDrag] = React.useState(null);
      const [immersive, setImmersive] = React.useState(false);
      const [defaultPreviewHeight] = React.useState(() => defaultHeight(typeof window === 'undefined' ? 620 : window.innerHeight));
      const previewHeight = layout.height === null ? defaultPreviewHeight : layout.height;
      const fillPreview = immersive && layout.height === null;
      // 搜索筛选：文件类型 / 时间范围 / 目录
      const [filterExt, setFilterExt] = React.useState('');
      const [filterDays, setFilterDays] = React.useState('');
      const [filterDir, setFilterDir] = React.useState('');
      const [resyncNote, setResyncNote] = React.useState(null);
      ...
      const runSearch = async () => {
        ...
        try {
          ...
          if (filterExt !== '') params.set('ext', filterExt);
          if (filterDays !== '') params.set('days', filterDays);
          if (filterDir.trim() !== '') params.set('dir', filterDir.trim());
          const value = await api(`/search?${params.toString()}`);
          setResult(value);
          setError(null);
        } catch (caught) {
          setError(caught.message);
      ...
      /** 只重抽指定文件（失败重试）：显式动作，跑完刷新状态与记录。 */
      const resyncFiles = async rels => {
        if (rels.length === 0) return;
        setResyncNote(`正在重抽 ${rels.length} 个文件…`);
        try {
          await api('/resync', { method: 'POST', body: { rels } });
          setResyncNote(`已提交重抽 ${rels.length} 个文件，跑完会在「同步记录」里看到结果`);
          setLogReload(value => value + 1);
        } catch (caught) {
          setResyncNote(`重抽失败：${caught.message}`);
        }
      };
      ...
      // 排版预览里的命中导航：iframe 同源，可以拿到 contentDocument 直接滚动
      const previewMarks = () => {
        try {
          const doc = iframeRef.current?.contentDocument;
          return doc === null || doc === undefined ? [] : [...doc.querySelectorAll('mark.kb-hit')];
        } catch {
          return [];
        }
      };
      const gotoHit = next => {
        ...
        const index = ((next % marks.length) + marks.length) % marks.length;
        setHitIndex(index);
        marks[index].scrollIntoView({ block: 'center', behavior: 'smooth' });
      };

      // ---- 预览布局：宽度可拖、高度可拖/可选档、满屏，选择记在 localStorage ----
      ...
      const beginDrag = kind => event => {
        if (kind === 'split' && (narrowViewport() || immersive)) return;
        if (kind === 'height' && fillPreview) return;
      ...
      const dragSplit = event => {
        ...
        const ratio = clampSplit(((event.clientX - rect.left) / rect.width) * 100);
        if (Math.abs(ratio - layoutRef.current.ratio) < 0.5) return;
        remember({ ...layoutRef.current, ratio });
      };
      const dragHeight = event => {
        if (drag !== 'height') return;
        const rect = previewWrapRef.current?.getBoundingClientRect();
        if (rect === undefined) return;
        const height = clampHeight(event.clientY - rect.top);
        if (Math.abs(height - (layoutRef.current.height ?? previewHeight)) < 4) return;
        remember({ ...layoutRef.current, height });
      };
      const resetSplit = () => remember({ ...layoutRef.current, ratio: SPLIT_DEFAULT });
      const resetHeight = () => remember({ ...layoutRef.current, height: null });
      const keySplit = event => {
        if (event.key !== 'ArrowLeft' && event.key !== 'ArrowRight' && event.key !== 'Home') return;
        event.preventDefault();
        if (event.key === 'Home') return resetSplit();
        const step = event.key === 'ArrowLeft' ? -2 : 2;
        remember({ ...layoutRef.current, ratio: clampSplit(layoutRef.current.ratio + step) });
      };
      const keyHeight = event => {
        if (event.key !== 'ArrowUp' && event.key !== 'ArrowDown' && event.key !== 'Home') return;
        event.preventDefault();
        if (event.key === 'Home') return resetHeight();
        const step = event.key === 'ArrowUp' ? -40 : 40;
        remember({ ...layoutRef.current, height: clampHeight((layoutRef.current.height ?? previewHeight) + step) });
      };
      // 满屏时按 Esc 退出
      React.useEffect(() => {
        if (!immersive || typeof window === 'undefined') return undefined;
      ...
      return h('div', { className: 'dslkb-root' },
        ...
        section === 'search' && h('div', { className: 'dslkb-card' },
          ...
          // 筛选：类型 / 时间 / 目录（改完要重新点检索）
          h('div', { className: 'dslkb-filters' },
            h('select', {
              className: 'dslkb-input',
              value: filterExt,
              title: '只看某类文件',
              ...
              h('option', { value: '' }, '全部类型'),
              h('option', { value: '.docx,.doc' }, 'Word（.docx/.doc）'),
              h('option', { value: '.xlsx,.xls' }, 'Excel（.xlsx/.xls）'),
              h('option', { value: '.pptx,.ppt' }, 'PPT'),
              h('option', { value: '.pdf' }, 'PDF'),
              h('option', { value: '.png,.jpg,.jpeg,.bmp,.tif,.tiff' }, '图片（OCR）'),
              h('option', { value: '.txt,.md,.csv,.json,.log' }, '文本')),
            h('select', {
              className: 'dslkb-input',
              value: filterDays,
              title: '只看最近改动的文件',
              ...
              h('option', { value: '90' }, '近 90 天'),
              h('option', { value: '365' }, '近一年')),
            h('input', {
              className: 'dslkb-input',
              value: filterDir,
              placeholder: '限定目录（可留空）',
              title: '只在这个目录下检索，例如：方案模板',
              onChange: event => setFilterDir(event.target.value),
              onKeyDown: event => { if (event.key === 'Enter') void runSearch(); },
            }),
            (filterExt !== '' || filterDays !== '' || filterDir.trim() !== '')
              ? h('button', {
                type: 'button',
                className: 'dslkb-btn',
                'data-shape': 'sm',
                'data-variant': 'ghost',
                onClick: () => { setFilterExt(''); setFilterDays(''); setFilterDir(''); },
              }, '清除筛选')
              : null),
          ...
          h('div', {
            ...
            // 只写 CSS 变量：窄屏的媒体查询才有机会把三列改回单列
            style: { '--kb-split': `${layout.ratio}%` },
          },
          ...
          // 中间那列是拖拽手柄：拖动改比例，双击复位，方向键微调
          openDoc !== null && !immersive && h('div', {
            className: 'dslkb-split-handle',
            role: 'separator',
            'aria-orientation': 'vertical',
            'aria-label': '调整结果列表与预览的宽度',
            'aria-valuenow': Math.round(layout.ratio),
            'aria-valuemin': SPLIT_RANGE[0],
            'aria-valuemax': SPLIT_RANGE[1],
            tabIndex: 0,
          ...
          openDoc !== null && h('div', { className: 'dslkb-preview dslkb-pane-detail', ref: previewRef },
            ...
            // 预览大小：档位 + 满屏 + 新标签页（拖拽手柄在预览区右下沿）
            (openDoc.found === true && (isPreviewType(openDoc.ext) || openDoc.ext === '.pdf'))
              ? h('div', { className: 'dslkb-hitnav' },
                h('span', { className: 'dslkb-hint' }, '预览大小'),
                h('div', { className: 'dslkb-seg', role: 'tablist' },
                  HEIGHT_PRESETS.map(preset => h('button', {
                    key: preset.key,
                    type: 'button',
                    className: 'dslkb-seg-btn',
                    role: 'tab',
                    'aria-selected': !fillPreview && previewHeight === preset.value ? 'true' : 'false',
                    title: `${preset.value}px`,
                    onClick: () => remember({ ...layoutRef.current, height: preset.value }),
                  }, preset.label)),
                  h('button', {
                    type: 'button',
                    className: 'dslkb-seg-btn',
                    role: 'tab',
                    'aria-selected': fillPreview ? 'true' : 'false',
                    title: '隐藏结果列表，预览占满窗口（Esc 退出）',
                    onClick: () => {
                      setImmersive(!immersive);
                      remember({ ...layoutRef.current, height: null });
                    },
                  }, immersive ? '退出满屏' : '满屏')),
                h('span', { className: 'dslkb-hint' }, fillPreview
                  ? '占满窗口高度'
                  : `${previewHeight}px${layout.height === null ? '（默认）' : ''}，可拖右下沿调整`),
                h('span', { className: 'dslkb-sep' }),
                h('a', {
                  className: 'dslkb-btn',
                  'data-shape': 'sm',
                  'data-variant': 'ghost',
                  href: `${API}/preview?rel=${encodeURIComponent(openDoc.rel)}`
                    + (query.trim().length > 0 ? `&q=${encodeURIComponent(query.trim())}` : ''),
                  target: '_blank',
                  rel: 'noreferrer',
                  title: '在浏览器新标签页里打开（整页显示）',
                }, h(IconExternal, { size: 14 }), '新标签页'))
              : null,
            ...
            openDoc.found === true && isInlineType(openDoc.ext)
              ? (openDoc.ext === '.pdf'
                ? h('div', {
                  className: `dslkb-preview-body${fillPreview ? ' dslkb-preview-fill' : ''}`,
                  ref: previewWrapRef,
                  style: { '--kb-preview-h': `${previewHeight}px` },
                },
                  h('iframe', {
                  ...
                  !fillPreview && h('div', {
                    className: 'dslkb-resize-y',
                    role: 'separator',
                    'aria-orientation': 'horizontal',
                    'aria-label': '调整预览高度',
                    'aria-valuenow': previewHeight,
                    'aria-valuemin': HEIGHT_RANGE[0],
                    'aria-valuemax': HEIGHT_RANGE[1],
                    tabIndex: 0,
                    title: '上下拖动调整高度（双击恢复默认，↑/↓ 微调）',
                    onPointerDown: beginDrag('height'),
                    onPointerMove: dragHeight,
                    onPointerUp: endDrag,
                    onPointerCancel: endDrag,
            ...
            openDoc.found === true && isPreviewType(openDoc.ext) && previewMode === 'preview'
              ? h(React.Fragment, null,
                query.trim().length > 0 && h('div', { className: 'dslkb-hitnav' },
                  h('span', { className: 'dslkb-hint' },
                    hitCount === null ? '正在统计命中…' : (hitCount === 0 ? '正文里没有直接命中（命中可能在表格/图片文字中）' : `命中 ${hitCount} 处${hitCount > 0 ? `（第 ${hitIndex + 1} 处）` : ''}`)),
                  h('span', { className: 'dslkb-sep' }),
                  h('button', {
                    type: 'button',
                    className: 'dslkb-btn',
                    'data-shape': 'sm',
                    'data-variant': 'ghost',
                    disabled: hitCount === null || hitCount === 0,
                ...
                h('div', {
                  ...
                  h('iframe', {
                    ...
                    src: `${API}/preview?rel=${encodeURIComponent(openDoc.rel)}`
                      + (query.trim().length > 0 ? `&q=${encodeURIComponent(query.trim())}#kb-hit-0` : ''),
                    title: `${openDoc.rel} 预览`,
                    // 只允许同源（图片要走同源 /media），脚本/表单/弹窗一律禁止；
                    // 宿主侧还会带 CSP 再兜一层
                    sandbox: 'allow-same-origin',
                    onLoad: () => { setHitCount(previewMarks().length); },
                  }),
                  !fillPreview && h('div', {
                    className: 'dslkb-resize-y',
                    role: 'separator',
        ...
        section === 'log' && h('div', { className: 'dslkb-card' },
          ...
          h('div', { className: 'dslkb-card-head', style: { marginTop: '4px' } },
            ...
            (status?.stats?.failures ?? []).length > 0 ? h('span', { className: 'dslkb-sep' }) : null,
            (status?.stats?.failures ?? []).length > 0
              ? h('button', {
                ...
                'data-variant': 'ghost',
                disabled: running,
                title: '只把这几个文件重新抽一遍（不动其它文件，不是整库重建）',
                onClick: () => void resyncFiles((status?.stats?.failures ?? []).map(item => item.rel)),
              }, '全部重抽')
              : null),
          resyncNote !== null ? h('div', { className: 'dslkb-note' }, resyncNote) : null,
          (status?.stats?.failures ?? []).length === 0
              ...
              (status?.stats?.failures ?? []).slice(0, 20).map(item => h('div', {
                ...
                h('div', { className: 'dslkb-hit-head' },
                  ...
                  h('button', {
                    ...
                    'aria-label': '重抽这个文件',
                    disabled: running,
                    onClick: () => void resyncFiles([item.rel]),
    ...
    function SettingsPage() {
      ...
      return h('div', { className: 'dslkb-root' },
        ...
        h('div', { className: 'dslkb-card' },
          ...
          check('ocrEmbeddedImages', '识别文档内嵌图片里的文字（架构图/界面截图也能搜到；首次较慢，之后走缓存）', undefined, true),

          h('div', { className: 'dslkb-field' },
            h('span', { className: 'dslkb-field-label' }, '内嵌图片'),
            h('div', { className: 'dslkb-field-body' },
              h('input', {
                className: 'dslkb-input',
                type: 'number',
                min: 0,
                step: 4,
                defaultValue: Math.round((settings.embeddedImageMinBytes ?? 20480) / 1024),
                onBlur: event => void patch({ embeddedImageMinBytes: Math.max(0, Number(event.target.value) || 0) * 1024 }),
              }),
              h('span', { className: 'dslkb-hint' }, 'KB 以下当 logo/图标跳过'),
              h('input', {
                className: 'dslkb-input',
                type: 'number',
                min: 0,
                step: 10,
                defaultValue: settings.maxEmbeddedImagesPerFile ?? 40,
                onBlur: event => void patch({ maxEmbeddedImagesPerFile: Math.max(0, Number(event.target.value) || 0) }),
              }),
              h('span', { className: 'dslkb-hint' }, '单文件最多识别？张'))),
    ...
    // LogDetails / 布局纯函数一并导出：只给单测用（浏览器里只用 apply 与 inject）
    module.exports = {
      apply,
      inject,
      LogDetails,
