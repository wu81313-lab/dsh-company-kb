        ...
        for (const item of changed) {
          ...
          try {
            ...
            if ((file.tooLarge && !ocrKind) || kind === 'binary' || !useContent) {
              ...
              summary.skipped += 1;
              if (reason === 'added') summary.added += 1; else summary.updated += 1;
            ...
            summary.failed += 1;
            summary.errors.push(`${file.rel}: ${error.message}`);
            try {
              upsertMetadata(file, {
        ...
        for (let index = 0; index < ocrJobs.length; index += OCR_BATCH) {
          ...
          try {
            ...
            for (const job of batch) {
              summary.failed += 1;
              summary.errors.push(`${job.file.rel}: ${error.message}`);
          ...
          for (const job of batch) {
            ...
            try {
              if (!result.ok || text.trim().length === 0) {
                summary.failed += 1;
                summary.errors.push(`${job.file.rel}: ${result.error ?? 'OCR 未识别出文本'}`);
                upsertMetadata(job.file, {
                ...
                if (job.previous === undefined) summary.added += 1; else summary.updated += 1;
              }
            } catch (error) {
              summary.failed += 1;
              summary.errors.push(`${job.file.rel}: ${error.message}`);
          ...
          for (const item of docxMediaJobs) {
            ...
            try {
              if (parts.length === 0) {
                ...
                summary.chars += text.length;
                if (item.previous === undefined) summary.added += 1; else summary.updated += 1;
              }
            } catch (error) {
              summary.failed += 1;
              summary.errors.push(`${item.file.rel}: ${error.message}`);
            }
          }
        }
          ...
          for (const [position, job] of wordJobs.entries()) {
            ...
            try {
              if (!result.ok || result.text.trim().length === 0) {
                summary.failed += 1;
                summary.errors.push(`${job.file.rel}: ${result.error ?? '未取到文本'}`);
                upsertMetadata(job.file, {
