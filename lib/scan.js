...
import { extractDocxImages, extractEmbeddedImages } from './extract/media.js';

const OCR_BATCH = 40;

/** 内嵌图片可能是内容的格式（其余格式扫图没意义）。 */
const embeddedImageExts = new Set(['.docx', '.docm', '.pptx', '.xlsx', '.xlsm']);
...
export function createSyncEngine({ settings, store, ocr, logger = console, onProgress = () => {} }) {
  ...
  async function run(mode = 'now', { only } = {}) {
    if (activeRun !== null) return activeRun;
    activeRun = (async () => {
      cancelRequested = false;
      Object.assign(state, {
        running: true,
        mode: Array.isArray(only) && only.length > 0 ? 'retry' : mode,
        phase: 'scan',
        total: 0,
        done: 0,
        current: '',
      ...
      // only：只重抽这些文件（失败重试用），其它文件一律按"未变化"处理
      const onlySet = Array.isArray(only) && only.length > 0 ? new Set(only.map(String)) : null;
      const effectiveMode = onlySet === null ? mode : 'retry';
      const summary = {
        mode: effectiveMode,
        ...
        ocrPages: 0,
        wordFiles: 0,
        embeddedImages: 0,      // 本次纳入检索的内嵌图片数（含命中缓存）
        embeddedCached: 0,
        chars: 0,
        errors: [],
        durationMs: 0,
      ...
      const pendingMedia = [];
      const embeddedJobs = [];   // 有正文的文档：内嵌图片等待 OCR

      /** 抽取文档内嵌图片并按 OCR 缓存分拣成"已缓存 / 待识别"两拨。 */
      const collectEmbeddedImages = (file, settings, store) => {
        const media = extractEmbeddedImages(file.path, file.ext, {
          maxCount: settings.maxEmbeddedImagesPerFile,
          minBytes: settings.embeddedImageMinBytes,
        });
        const cached = [];
        const pending = [];
        for (const image of media.files) {
          const known = store.getImageOcr(image.sha1);
          if (known === undefined) pending.push(image);
          else if (known.trim().length > 0) cached.push({ entry: image.entry, text: known, sha1: image.sha1 });
        }
        return { media, cached, pending };
      };

      /** 图片 OCR 文字 → 独立块（带【图片N】标记，方便引用时说清来源）。 */
      const embeddedBlocks = (items) => items
        .filter(item => String(item.text ?? '').trim().length > 0)
        .map((item, index) => ({
          kind: 'para',
          text: `【图片${index + 1}】${String(item.text).replace(/\s+/gu, ' ').trim()}`,
        }));

      const blocksChars = (blocks) => blocks.reduce((sum, block) => sum + String(block.text ?? '').length, 0);
      const warningsOf = (extracted) => (Array.isArray(extracted.meta?.warnings) && extracted.meta.warnings.length > 0
      ...
      try {
        ...
        for (const file of files) {
          ...
          if (onlySet !== null) {
            if (onlySet.has(String(file.rel)) || onlySet.has(String(file.path))) {
              changed.push({ file, reason: 'retry', previous });
            } else {
              summary.unchanged += 1;
            }
            continue;
          ...
          // 开关打开后，老索引里的 Office 文件还没做过"内嵌图片 OCR"：
          // 逐个补做一次即可，不需要整库重建（做完会把 media_ocr 置 1）
          if (current.ocrEmbeddedImages !== false
            && embeddedImageExts.has(String(file.ext).toLowerCase())
            && Number(previous.media_ocr) !== 1) {
            changed.push({ file, reason: 'media', previous });
            continue;
          }
          summary.unchanged += 1;
        }
        ...
        const upsertMetadata = (file, { kind, status, textSource, error, reason, previous }) => {
          ...
          store.upsertDocument({
            ...
            // 这类文件已经"决定了怎么处理"，标记一下，避免每次同步都被重新抽图
            mediaOcr: true,
          }, [chunk], { trigram });
            ...
            if ((file.tooLarge && !ocrKind) || kind === 'binary' || !useContent) {
              ...
              if (extracted.needs === 'ocr') {
                ...
                if (chunks.length === 0 && kind === 'docx') {
                  ...
                  // 有正文，但方案书/投标文件里的架构图与截图上的字本来是搜不到的。
                  // 开启 ocrEmbeddedImages 时把它们也抽出来 OCR，文字作为独立块追加。
                  const embedded = current.ocrEmbeddedImages !== false
                    ? collectEmbeddedImages(file, current, store)
                    : null;
                  if (embedded !== null && (embedded.pending.length > 0 || embedded.cached.length > 0)) {
                    pendingMedia.push(embedded.media);
                    state.total += embedded.pending.length;
                    if (embedded.pending.length > 0) {
                      embeddedJobs.push({
                        file,
                        previous,
                        kind,
                      ...
                      // 图片全在缓存里：不用等 OCR，直接带图文字入库
                      const imageBlocks = embeddedBlocks([...embedded.cached]);
                      store.upsertDocument({
                        ...
                        textSource: 'native', chars: extracted.text.length + blocksChars(imageBlocks),
                        pages: extracted.meta?.pageCount,
                        status: 'indexed',
                        error: warningsOf(extracted),
                        tries: 0,
                        mediaOcr: true,
                      }, chunkBlocks([...extracted.blocks, ...imageBlocks], {
                        chunkChars: current.chunkChars,
                        chunkOverlap: current.chunkOverlap,
                      }), { trigram });
                      noteChanged(file.rel, reason === 'added');
                      summary.chars += extracted.text.length + blocksChars(imageBlocks);
                      summary.embeddedImages += embedded.cached.length;
                    }
                  } else {
                    store.upsertDocument({
                      path: file.path, rel: file.rel, dir: file.dir, name: file.name, ext: file.ext,
                      ...
                      error: warningsOf(extracted),
                      tries: 0,
                      // 开关关着时不要标记，这样以后打开开关还能给这些文件补做
                      mediaOcr: current.ocrEmbeddedImages !== false,
                    }, chunks, { trigram });
                    noteChanged(file.rel, reason === 'added');
        ...
        // —— 有正文的文档：内嵌图片 OCR。
        // 这些图里的字（架构图标签、界面截图、表格截图）本来完全搜不到，识别出来后
        // 作为独立块追加到文档末尾；结果按图片 sha1 写进缓存，下次重建直接命中。
        if (embeddedJobs.length > 0 && !cancelRequested) {
          const jobs = [];
          for (const item of embeddedJobs) {
            item.pending.forEach((image, index) => {
              jobs.push({ id: `${item.file.path}::${index}`, kind: 'image', path: image.path });
            });
          }
          state.phase = 'ocr';
          state.current = `内嵌图片 OCR（${jobs.length} 张）`;
          emit();
          const results = new Map();
          if (ocr.capabilities?.ocr !== true) {
            summary.errors.push(`内嵌图片 OCR 跳过：本机 OCR 不可用（涉及 ${embeddedJobs.length} 个文件）`);
          } else {
            for (let index = 0; index < jobs.length; index += OCR_BATCH) {
              if (cancelRequested) break;
              const batch = jobs.slice(index, index + OCR_BATCH);
              try {
                ...
                summary.errors.push(`内嵌图片 OCR 失败：${error.message}`);
              }
              state.done += batch.length;
              state.current = `内嵌图片 OCR ${Math.min(index + batch.length, jobs.length)}/${jobs.length}`;
              emit();
            }
          }

          for (const item of embeddedJobs) {
            const collected = [...item.cached];
            item.pending.forEach((image, index) => {
              const result = results.get(`${item.file.path}::${index}`);
              const text = result !== undefined && result.ok === true ? String(result.text ?? '').trim() : '';
              // 空结果也缓存：避免每次同步都对同一张装饰图重复 OCR
              try { store.setImageOcr(image.sha1, text, image.bytes); } catch { /* 缓存写失败不影响入库 */ }
              if (text.length > 0) collected.push({ entry: image.entry, text, sha1: image.sha1 });
            });
            const imageBlocks = embeddedBlocks(collected);
            try {
              const blocks = [...item.extracted.blocks, ...imageBlocks];
              store.upsertDocument({
                path: item.file.path, rel: item.file.rel, dir: item.file.dir, name: item.file.name,
                ext: item.file.ext, size: item.file.size, mtime: item.file.mtime,
                sha1: sha1Of(item.file.path), kind: item.kind, textSource: 'native',
                chars: item.extracted.text.length + blocksChars(imageBlocks),
                pages: item.extracted.meta?.pageCount,
                status: 'indexed', error: warningsOf(item.extracted), tries: 0,
                mediaOcr: true,
              }, chunkBlocks(blocks, {
                chunkChars: current.chunkChars,
                chunkOverlap: current.chunkOverlap,
              }), { trigram });
              summary.embeddedImages += collected.length;
              summary.embeddedCached += item.cached.length;
              summary.chars += item.extracted.text.length + blocksChars(imageBlocks);
              noteChanged(item.file.rel, item.previous === undefined);
            } catch (error) {
              noteFailed(item.file.rel, error.message);
            }
        ...
        store.logSync(
          ...
          `同步(${summary.mode}) 扫描 ${summary.scanned}，新增 ${summary.added}，更新 ${summary.updated}，删除 ${summary.removed}，跳过 ${summary.skipped}，失败 ${summary.failed}，耗时 ${summary.durationMs}ms`
            + (summary.embeddedImages > 0 ? `；内嵌图片 ${summary.embeddedImages} 张（缓存命中 ${summary.embeddedCached}）` : '')
