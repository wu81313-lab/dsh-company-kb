 ...
 */
export function renderDocumentHtml({ buffer, ext, rel, mediaBase }) {
  const mediaUrl = name => `${mediaBase}?rel=${encodeURIComponent(rel)}&name=${encodeURIComponent(name)}`;
  let result;
  if (ext === '.docx') result = renderDocxHtml(buffer, mediaUrl);
  else if (ext === '.xlsx' || ext === '.xlsm') result = renderXlsxHtml(buffer);
  else if (ext === '.pptx') result = renderPptxHtml(buffer, mediaUrl);
