...
function sheetHtml(sheet, parsed, highlight) {
  ...
  for (const rowIndex of rowIndexes) {
    ...
    for (let col = 0; col < colCount; col += 1) {
      ...
      parts.push(`<td${attrs}>${highlight(escapeHtml(value))}</td>`);
    }
...
export function renderXlsxHtml(buffer, { highlight = text => text } = {}) {
  ...
  for (const sheet of targets) {
    ...
    const section = sheetHtml(sheet, parsed, highlight);
